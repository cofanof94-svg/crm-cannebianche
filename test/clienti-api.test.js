...
async function makeApp() {
  ...
  const crmDb = {
    async query(text, params) {
      ...
      if (/FROM customer_notes/.test(text)) return note.filter((n) => ids.includes(n.pmsCustomerId)).map((n) => ({ id: n.id, testo: n.testo, autore: 'admin', created_at: 'x', autore_user_id: 1, pms_customer_id: n.pmsCustomerId }));
...
test('merge: nota su un duplicato appare nel gruppo; unmerge la scollega', async () => {
  const app = await makeApp();
  const ag = await agente(app);
  // fondo 55491 in 47186, poi scrivo una nota sul duplicato 55491
  const m = await ag.post('/api/clienti/47186/merge').send({ memberId: 55491, canonicalId: 47186 });
  assert.strictEqual(m.status, 201);
  await ag.post('/api/clienti/55491/note').send({ testo: 'nota sul duplicato' });
  // vista dalla scheda 47186 (principale) la nota del gruppo compare
  let res = await ag.get('/api/clienti/47186/note');
  assert.strictEqual(res.body.note.length, 1);
  assert.strictEqual(res.body.note[0].testo, 'nota sul duplicato');
  ...
  // annullo la fusione → la nota non è più nel gruppo di 47186
  const del = await ag.delete('/api/merge/55491');
  assert.strictEqual(del.status, 200);
  res = await ag.get('/api/clienti/47186/note');
  assert.strictEqual(res.body.note.length, 0);
});
